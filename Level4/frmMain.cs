...
namespace Level4
{
    public partial class frmMain : Form
    {
        ...
        private void btnEqual_Click(object sender, EventArgs e)
        {
            ...
            enumerator.MoveNext();
            // 要素リストの最初は数値のはず
            double left = (enumerator.Current as InputNumber).Value;

            // 通常はforeachで回すが２つずつ取り出したいのでイテレータを使う
            // 最初の数値は取り出してあるので残りは演算子と数値の組み合わせで偶数個あるはず
            while(enumerator.MoveNext())
            {
                Operator ope = enumerator.Current as Operator;
                enumerator.MoveNext();
                double right = (enumerator.Current as InputNumber).Value;
                left = ope.Calculate(left, right);
            }
            txtExpression.Text = $"{MakeExpression()} ＝ {left.ToString().Replace(".", "．")}";
